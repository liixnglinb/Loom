...
PALETTES = {
    ...
    # 顶刊风格 (Water Research / Nature 级别) — 低饱和灰调，SCI 投稿首选
    ...
    # ==== 知名配色库（GitHub 公认 · 好看易用） ====
    'urban':        ['#1696D2', '#FDBF11', '#55B748', '#DB2B27', '#EC008B', '#00A3A1', '#8C6DB1', '#7F8C8D'],  # Urban Institute
    'sci_std':      ['#0C5DA5', '#00B945', '#FF9500', '#FF2C00', '#845B97', '#474747', '#9E9E9E'],  # SciencePlots std
    'kelly':        ['#F3C300', '#875692', '#F38400', '#A1CAF1', '#BE0032', '#C2B280', '#848482', '#008856', '#E68FAC', '#0067A5'],  # Kelly 1965
    'plasma_bright':['#2196F3', '#FFC107', '#4CAF50', '#F44336', '#9C27B0', '#00BCD4', '#FF9800', '#E91E63'],  # Material 亮色
}

# 「随机模式」可抽取的配色池(不含 nature/npg 等有专属逻辑或过于特殊的,只放适合通用随机的)
RANDOM_PALETTE_POOL = [
    'elegant', 'okabe_ito', 'tol_muted', 'tol_vibrant', 'nord', 'sunburst', 'ocean',
    ...
    'urban', 'sci_std', 'kelly', 'plasma_bright',
]
...
STYLE_FAMILIES = {
    ...
    'sci_frame':      {'spines': ('left', 'bottom', 'top', 'right'), 'tick_dir': 'in', 'tick_tr': True,
                       'grid': {'axis': 'both', 'ls': ':', 'alpha': 0.12, 'color': '#DDDDDD'},
                       'legend_frame': True, 'patch_edge': 'white', 'lw': 1.8, 'ms': 5, 'axis_color': '#333333'},
    'soft_mesh':      {'spines': ('left', 'bottom'), 'tick_dir': 'out', 'tick_tr': False,
                       'grid': {'axis': 'both', 'ls': ':', 'alpha': 0.18, 'color': '#CCCCCC'},
                       'legend_frame': False, 'patch_edge': 'white', 'lw': 1.8, 'ms': 5, 'axis_color': '#666666'},
    'bold_panel':     {'spines': ('left', 'bottom', 'top', 'right'), 'tick_dir': 'out', 'tick_tr': False,
                       'grid': None, 'legend_frame': False, 'patch_edge': 'white',
                       'lw': 2.2, 'ms': 6, 'axis_color': '#222222'},
    'airy_open':      {'spines': ('left', 'bottom'), 'tick_dir': 'out', 'tick_tr': False,
                       'grid': None, 'legend_frame': False, 'patch_edge': 'none',
                       'lw': 1.6, 'ms': 4, 'axis_color': '#999999'},
    'dotted_grid':    {'spines': ('left', 'bottom'), 'tick_dir': 'out', 'tick_tr': False,
                       'grid': {'axis': 'both', 'ls': '.', 'alpha': 0.22, 'color': '#BBBBBB'},
                       'legend_frame': False, 'patch_edge': 'white', 'lw': 1.8, 'ms': 5, 'axis_color': '#555555'},
    'journal_compact':{'spines': ('left', 'bottom', 'top', 'right'), 'tick_dir': 'in', 'tick_tr': True,
                       'grid': None, 'legend_frame': True, 'patch_edge': 'white',
                       'lw': 1.7, 'ms': 4.5, 'axis_color': '#444444'},
}
...
def _save(fig, output):
    ...
    # ★ 矢量双存档：PDF（LaTeX 论文直接引用）外，必同存一份同名 .svg 主存档——
    #   保证"最终论文里的图都是 SVG 级矢量"，svg 供复用/再排版，两者同为矢量、放大无损。
    if _ext == 'pdf' and output.lower().endswith('.pdf'):
        _svg_out = output[:-4] + '.svg'
        try:
            fig.savefig(_svg_out, format='svg', bbox_inches=None, pad_inches=0.15)
            print(f'Saved: {_svg_out} (SVG 主存档)')
        except Exception as _e:
            print(f'WARN: SVG 主存档失败，PDF 矢量图不受影响: {_e}')
