...
for script in figures/gen_fig*.py; do
    ...
    # ⛔ 论文图必须矢量：save_fig/savefig 一律存 .pdf（自动并列写 SVG 主存档，放大不糊）。
    #    禁止直接落 .png/.jpg/.jpeg（仅 docx 导出链路允许 PNG，PDF 论文禁用位图）。
    RASTER_SAVE=$(grep -oE "save_fig\([^)]*['\"][^'\"]+\.(png|jpg|jpeg)['\"]|savefig\([^)]*['\"][^'\"]+\.(png|jpg|jpeg)['\"]" "$script" 2>/dev/null | head -3)
    if [ -n "$RASTER_SAVE" ]; then
        echo "WARNING $bn: 保存了位图(${RASTER_SAVE}) — 最终 PDF 论文的图必须是 SVG/PDF 矢量："
        echo "         把输出改为 .pdf（plot_utils 会自动同存 .svg 主存档）：save_fig(fig, 'figures/fig_xxx.pdf')"
        [ "${STOP_RASTER:-0}" = "1" ] && recipe_issues=$((recipe_issues+1))
    fi
