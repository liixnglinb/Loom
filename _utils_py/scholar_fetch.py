...
_USER_AGENT = "ModelFlow-ScholarFetch/1.0 (academic research tool)"
_TIMEOUT = 15  # 单次请求超时秒数
_RATE_DELAY = 0.3  # 请求间隔（秒），避免被限流

# Windows 上 Python 可能缺少 CA 证书，创建不验证 SSL 的 context 作为 fallback
import ssl as _ssl
try:
    _SSL_CTX = _ssl.create_default_context()
except Exception:
    _SSL_CTX = _ssl.create_default_context()
# 如果默认 context 验证失败，用不验证的 context
...
def _http_get(url: str, headers: Optional[dict] = None, timeout: int = _TIMEOUT) -> Optional[str]:
    ...
    - 其他 SSLError (UNEXPECTED_EOF / handshake 失败) 也走 noverify
    """
    ...
    try:
        ...
        # 所有 SSL 错误 (cert / EOF / handshake) 都走 noverify
        try:
            with urllib.request.urlopen(req, timeout=timeout, context=_SSL_CTX_NOVERIFY) as resp:
                return resp.read().decode("utf-8", errors="replace")
        except Exception as e2:
            _log(f"HTTP GET failed (SSL fallback): {url} -> {e2}")
            return None
    except (urllib.error.URLError, urllib.error.HTTPError, OSError, TimeoutError) as e:
        # URLError 可能包装了 SSLError
        emsg = str(e)
        if ("CERTIFICATE_VERIFY_FAILED" in emsg or "SSL" in emsg
                or "EOF" in emsg or "ssl" in emsg.lower()):
            try:
                with urllib.request.urlopen(req, timeout=timeout, context=_SSL_CTX_NOVERIFY) as resp:
                    return resp.read().decode("utf-8", errors="replace")
            except Exception as e2:
...
def _http_post(url: str, body: dict, headers: Optional[dict] = None, timeout: int = _TIMEOUT) -> Optional[str]:
    ...
    try:
        ...
        try:
            ...
            _log(f"HTTP POST failed (SSL fallback): {url} -> {e2}")
            return None
    except (urllib.error.URLError, urllib.error.HTTPError, OSError, TimeoutError) as e:
        emsg = str(e)
        if ("CERTIFICATE_VERIFY_FAILED" in emsg or "SSL" in emsg
...
# AMiner API Key — 内置默认 key（datacenter.aminer.cn 官方平台），用户无需配置
# ⛔ 修复用户反馈"很多搜不到"的 bug:
# - 旧 endpoint api.302ai.cn (第三方网关) 在 Windows schannel 下 TLS 握手失败,
#   全军覆没。改为直连 AMiner 官方 datacenter.aminer.cn。
# - URL 路径从 /aminer/gateway/... 改为 /gateway/...
# - Authorization 头不再加 "Bearer " 前缀, 直接传 token (官方约定)
# - 旧 token 已失效返回 40308, 用新 token (有效期到 2108 年)
_AMINER_API_KEY = os.environ.get(
    "AMINER_API_KEY",
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJleHAiOjQzNzI3MzA2MTEsInRpbWVzdGFtcCI6MTc4MDczMDYxMSwidXNlcl9pZCI6IjZhMjNjOTU0ZDRkMzMyZjZjOWI1OGRjOCJ9.eD_W3DCHWQy5LODNNRutt9xceXq6hRAuAoTtu_Kc2Lo",
)
...
def openalex_search(query: str, max_results: int = 5) -> list[dict]:
    """搜索 OpenAlex（2.5亿+ 论文，免费无 Key，人文社科/多语种/中文覆盖最广）。

    作为 AMiner/S2/DBLP/CrossRef 全部无结果时的兜底源，降低"搜不到→编造文献"的概率。
    API 文档: https://docs.openalex.org/  返回真实 DOI，后续 BibTeX 仍走 CrossRef DOI。
    """
    params = urllib.parse.urlencode({
        "search": query,
        "per_page": min(max(max_results, 1), 25),
        "sort": "relevance_score:desc",
    })
    url = f"https://api.openalex.org/works?{params}"
    # mailto 进入 polite pool（更稳定），不暴露真实邮箱
    text = _http_get(url, headers={"User-Agent": "ModelFlow-scholar/1.0 (mailto:support@modelflow.local)"})
    if not text:
        return []
    try:
        data = json.loads(text)
...
def fetch_bibtex(query: str, max_results: int = 10) -> list[dict]:
    ...
    # 仍无结果 → OpenAlex 兜底（2.5亿+，人文社科/多语种/中文覆盖最广，防"搜不到→编造"）
    if not papers:
        _log("CrossRef returned no results, trying OpenAlex...")
        oa_results = openalex_search(query, max_results=max_results)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "search":
        ...
        if not results:
            # 最终兜底 OpenAlex（人文社科/多语种/中文覆盖最广，防"搜不到→编造"）
            results = openalex_search(args.query, max_results=args.max)
