...
- `ext#N` → `_utils/figure_recipes_ext.md`（v0.8.5 新增 25 种图型，见文末《扩展图型索引》）
...
## 扩展图型索引（v0.8.5 新增 25 种，画法出处 ext#N → figure_recipes_ext.md）

### 比较类
| 图名 | 适配判定 | 画法出处 |
| --- | --- | --- |
| 气泡图 | 三/四维比较（气泡大小/颜色编码额外维度） | ext#1 |

### 趋势类
| 图名 | 适配判定 | 画法出处 |
| --- | --- | --- |
| 收敛曲线 | 启发式算法迭代最优值下降，证明收敛 | ext#2 |
| 阶梯图 | 离散跳变（阈值/补货/分段费率） | ext#3 |
| 事件时间线 | 关键里程碑/异常时点标注 | ext#4 |
| 学习曲线 | 训练/验证损失，判断过拟合与早停 | ext#5 |
| 灰色预测 GM(1,1) | 小样本趋势外推 + 预测区间 | ext#6 |
| 蒙特卡洛模拟带 | 随机模拟中位数 + 95% 置信带 | ext#7 |

### 分布类
| 图名 | 适配判定 | 画法出处 |
| --- | --- | --- |
| 聚类谱系图 | 层次聚类（样本/变量系统树） | ext#8 |
| 分裂小提琴图 | 同类别两组左右半拼分布对比 | ext#9 |

### 相关 / 回归
| 图名 | 适配判定 | 画法出处 |
| --- | --- | --- |
| 预测-观测 1:1 | 模型验证散点贴 y=x + R² | ext#10 |

### 组成 / 多维
| 图名 | 适配判定 | 画法出处 |
| --- | --- | --- |
| 旭日图 | 多级占比（内外环层级） | ext#11 |
| 弦图 | 两两关联/流向权重 | ext#12 |
| 三维散点 | 三变量联合分布立体 | ext#13 |
| 三维柱状图 | 双因素取值高度对比 | ext#14 |
| 三维等高线 | 双参数响应面寻优 | ext#15 |

### 专业 / 高级
| 图名 | 适配判定 | 画法出处 |
| --- | --- | --- |
| ROC 曲线 + AUC | 二分类模型性能评价 | ext#16 |
| 混淆矩阵 | 分类错误结构热力 | ext#17 |
| Pareto 前沿 | 多目标优化解集折衷 | ext#18 |
| 中国省区地图 | 地理分布填色（疫情/物流/灾害） | ext#19 |
| 流线矢量场 | 风场/流速场流向密度 | ext#20 |
| 特征重要性 | 模型变量贡献排序 | ext#21 |
| 神经网络结构 | AI 模型结构示意 | ext#22 |
| 决策树 | 分类判定树状 | ext#23 |
| 模糊隶属函数 | 模糊综合评价（低/中/高 隶属度） | ext#24 |
| AHP 判断矩阵 | 层次分析法两两比较权重 | ext#25 |
