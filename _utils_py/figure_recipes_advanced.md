...
## 35. Significance Bar — 显著性柱状图（误差棒 + 星号 + 显著性连线）
...
## 36. Forest Plot — 森林图（效应量/回归系数 + CI 汇总）
...
## 37. CUSUM Chart — 累积和图（过程漂移/突变点监测）
...
## 38. Quantile Trend Band — 分位数趋势带（mean±Q1/Q3 平滑带）
...
## 41. Joint Plot — 联合分布图（散点 + 边缘直方/密度）
...
## 42. Residual Diagnostics — 残差诊断四联图（回归模型体检）
...
## 43. Significance Heatmap — 相关显著性热力图（数值 + p 值星号）
...
## 44. Tornado Chart — 灵敏度龙卷风图（参数影响排序）
...
## 46. Phase Portrait — 相平面图（微分方程动力学）
...
## 47. Parameter Sweep Heatmap — 参数扫描热力图（双参数灵敏度）
...
## 48. Trajectory / OD Flow — 轨迹与流向图（空间路径）

**场景**: 路径规划/选址/物流题：在经纬度或网格坐标上画车辆/站点轨迹、或起讫 OD 流向箭头。与地图底图叠加最像论文成品。
**⚠ 关键要点**: 坐标按经纬度设定 aspect 校正（1/cos(lat)）避免变形；OD 流用带箭头曲线（FancyArrowPatch，曲率区分方向）；起点终点用不同图例。
...
routes = [  # (起点, 终点) 或轨迹折线
    np.array([[116.3, 39.9], [116.5, 40.1], [116.7, 39.85], [116.9, 40.0]]),
    np.array([[116.3, 39.9], [116.45, 39.7], [116.7, 39.75], [116.9, 39.55]]),
]
fig, ax = plt.subplots(figsize=(6.4, 5.0))
for i, tr in enumerate(routes):
    ax.plot(tr[:, 0], tr[:, 1], color=PALETTE[i], lw=1.8, alpha=.85, label=f'Route {i+1}')
    ax.scatter(tr[0, 0], tr[0, 1], s=30, color=PALETTE[i], marker='o', zorder=3)
    ax.scatter(tr[-1, 0], tr[-1, 1], s=30, color=PALETTE[i], marker='s', zorder=3)
    ax.annotate('', xy=tr[-1], xytext=tr[-2],
                arrowprops=dict(arrowstyle='-|>', color=PALETTE[i], lw=1.6))
ax.set_xlabel('Longitude'); ax.set_ylabel('Latitude')
ax.set_aspect(1/np.cos(np.deg2rad(39.9)))            # 纬度校正
ax.legend(frameon=False, fontsize=9)
ax.spines[['top', 'right']].set_visible(False)
fig.tight_layout()
save_fig(fig, 'figures/fig_traj.pdf')
```

---

## 49. Sequence Diagram — 时序图（HTML 引擎模板）
...
## 50. State Diagram — 状态机图（HTML 引擎模板）
...
## 52. Indicator Tree — 指标体系树（HTML 引擎模板）
