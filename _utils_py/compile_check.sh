...
# N+2. 论文图必须矢量（最终 PDF 里的图 = SVG/PDF 矢量，放大不糊）
#    硬闸：latex_includes.tex 与正文章节里 \includegraphics 引用 .png/.jpg/.jpeg → FAIL。
echo "--- 图必须矢量（禁位图入论文）---"
RASTER_FIGS=$(grep -rhoE '\\includegraphics(\[[^]]*\])?\{[^}]*\.(png|jpg|jpeg)\}' figures/latex_includes.tex "$PAPER_DIR"/sections/*.tex "$PAPER_DIR"/main.tex 2>/dev/null | sort -u | head -6)
if [ -n "$RASTER_FIGS" ]; then
    echo "  FAIL: 论文里存在位图引用(${RASTER_FIGS}) — 最终 PDF 的所有图必须是 SVG/PDF 矢量："
    echo "    修复：用 save_fig(fig, 'figures/fig_xxx.pdf') 重新出图（plot_utils 自动同存 .svg 主存档），"
    echo "          并把 \\includegraphics 路径改回 .pdf；示意图走 paper-figure-html(Chrome→矢量 PDF)。"
    EXIT_CODE=1
else
    echo "  OK: 论文全部 \\includegraphics 均为矢量(.pdf/.svg)"
fi
