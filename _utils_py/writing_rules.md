...
- **⛔ 所有图使用 `keepaspectratio` + `height` 双约束**，LaTeX 自动处理高图缩放，不需要在 HTML 引擎 层面限制图的高度
