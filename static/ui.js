...
const DICT = {
  zh: {
    ...
    'st.heat': 'Token 活动', 'st.heatGrp': '按天分桶', 'st.less': '少', 'st.more': '多',
    'st.tokens': '累计 Token', 'st.tokensD': '两个 CLI 自报的用量合计，软件不直连模型、拿不到就是 0。',
    'st.tokIO': '输入 / 输出', 'st.tokIOD': '输入含提示与上下文，输出是智能体真正写出的量。',
    'st.tokCache': '缓存读取', 'st.tokCacheD': '命中缓存的部分，通常远大于真实输入。',
    'st.tokReason': '推理', 'st.tokReasonD': '思考链消耗，只有支持的引擎才报。',
    'unit.yi': '亿', 'unit.wan': '万',
  ...
  en: {
    ...
    'st.heat': 'Token activity', 'st.heatGrp': 'Per day', 'st.less': 'Less', 'st.more': 'More',
    'st.tokens': 'Total tokens', 'st.tokensD': 'Summed from what the CLIs report. Loom never calls a model, so nothing reported counts as 0.',
    'st.tokIO': 'Input / output', 'st.tokIOD': 'Input covers prompts and context; output is what the agent actually wrote.',
    'st.tokCache': 'Cache read', 'st.tokCacheD': 'Cache hits, usually far larger than real input.',
    'st.tokReason': 'Reasoning', 'st.tokReasonD': 'Chain-of-thought spend, only engines that report it.',
    'unit.yi': 'B', 'unit.wan': 'K',
