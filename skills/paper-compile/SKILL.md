...
# TikZ 嵌入检测: TikZ 由 paper-figure-html 编译成 figures/tikz_diagrams.pdf,
...
# TikZ 计划检测: 只在"实际生成了 tikz_*.pdf 产物"时才硬核对(规划提了但用 HTML 引擎 替代时不误报)
...
if [ "$TIKZ_PDF_TOTAL" -gt 0 ]; then
    ...
    [ "$TIKZ" -gt 0 ] && echo "✅ TikZ embedded" || echo "  ⚠ Plan mentions TikZ but no tikz_*.pdf found (may be replaced by HTML 引擎, non-blocking)"
else
