...
5. 技术路线**根据 `skip_drawio` 参数自动决定形式**：
   - `skip_drawio=False`（默认，画图）：4.2 章节用 `![技术路线图](figures/fig_roadmap.png)` 引用图片，不写 mermaid/html 源码
...
- 在本节文字描述各研究阶段后，**末尾加一行**：
  ...
  ```
- **禁止**在正文中输出 mermaid/html/PlantUML 源码块
- 后续 paper-figure-html 步骤会自动绘制 fig_roadmap.html 并导出 PNG
...
3. **文献检查**：
   ...
   - 格式是否符合 GB/T 7714
4. **技术路线图**：
   - 如果 `skip_drawio=False`：必须包含 `![技术路线图](figures/fig_roadmap.png)` 引用，且不应有 mermaid/html 源码块
...
**写完上面的图表清单后，在产出文档（PAPER_PLAN.md / PROBLEM_ANALYSIS.md / 等）的**最后**追加一个机器可读的清单区块。下游 paper-figure / paper-figure-html / 写作 SKILL / workflow_engine.py 都按此清单对账。少一张就触发 AUTO-RECOVER。**
...
**HTML 引擎 流程/架构图（paper-figure-html 产出 .html + .png/.pdf）：**
- fig_arch
- fig_flow_xxx
...
**总数：DATA=N, HTML=M, TIKZ=K, ALL=N+M+K**
<!-- END FIGURE_MANIFEST -->
```

⛔ **铁律：**
- **每条只写文件名主干**（不带 .py / .html / .png / .pdf 后缀）
