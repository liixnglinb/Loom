...
⛐ **人文社科图表 ≠ STEM 论文堆数据图**：
- 一篇人文社科论文图表数量通常 **0-3 张**，HTML（概念关系图/理论框架图）比数据图更常见
...
- **开启「理论框架图/示意图」**：规划 1-2 张示意图（概念关系图 `fig_framework`、分析框架图、
  研究路线图 `fig_roadmap` 等），列入 HTML 类。

**为每张规划的图列以下 5 项**（写进 OUTLINE.md 对应章节，方便 humanities-write 阶段对照）：
- **图号**：fig_xxx（数据图）/ fig_framework_xxx / fig_roadmap_xxx（HTML 类）
- **标题**：用于正文引用（如 "图 1：晚清报刊关键词词频分布"）
- **目的**：本图回答论证中的哪个具体问题
- **数据源**：来自 user_data/xxx 文件 / 用户在知网检索后提供 / 历史档案
- **图表类型 + 配方**：如 "Lollipop 图 (advanced#7)" 或 "概念关系图 (html 手绘)"
...
**总数：DATA=0, HTML=0, TIKZ=0, ALL=0**
<!-- END FIGURE_MANIFEST -->
```
⛔ 把计数改成实际规划数量（开启图表时 DATA/HTML 不能仍写 0）。
