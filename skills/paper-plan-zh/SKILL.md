...
⛔ **流程/架构图那一类的章节标题按实际引擎写**：CLAUDE.md 若声明「流程图引擎 = HTML」（默认），标题写「**HTML 流程/架构图**」；若声明「引擎 = HTML 引擎」，才写「**HTML 引擎 流程/架构图**」。两种关键词下游都能识别对账，但**要与 CLAUDE.md 声明的引擎一致**，别选了 HTML 还写 HTML 引擎。图名前缀（fig_arch / fig_flow / fig_roadmap）与引擎无关，不变。
