...
- **GPT Image 生图**：`python _utils/gpt_image.py --prompt "..." --output figures/fig_scene.png --lang zh --max-retries 3`
  ...
  - 必须含图例框
  - 技术路线图用 HTML 引擎/TikZ，不用 GPT Image
...
**"加一张技术路线图"**：在 `figures/fig_roadmap.html` 设计 → 用 `screenshot_capture.py` 截图导出 PNG → 在主 .md 里 `![技术路线](figures/fig_roadmap.png)`
