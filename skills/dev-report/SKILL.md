...
报告的"系统设计/实现"章节需要**架构图、ER 图、流程图**。先用 HTML 出图引擎画出来存 `figures/`，正文再引用真图。
...
### 出图方式：HTML + CSS 自动布局引擎（同 paper-figure-html）
不手写坐标，用 flex/grid 描述结构关系；每张图一个 HTML 文件放 `figures/`，再截图导出 PDF/PNG。
...
- 文件命名固定：`fig_arch.html`、`fig_er.html`、`fig_flow_1.html`；显式声明 `charset=UTF-8` 保证中文，配色克制（≤3 色），相对布局天然免疫节点重叠/连线穿越。

### 导出图片（HTML→PDF/PNG，非桌面环境导出失败就退回占位符，不阻塞报告）
```bash
echo "=== 图表导出 ==="
...
for src in figures/fig_*.html; do
  [ -f "$src" ] || continue
  bn=$(basename "$src" .html)
  out="figures/${bn}.${FMT}"
  # HTML→PDF/PNG 截图导出（同 paper-figure-html 的 screenshot_capture.py；无 Electron 时自动降级）
  "$MH_PYTHON" _utils/screenshot_capture.py --file "$src" --out "$out" --format "$FMT" >/dev/null 2>&1
  if [ -f "$out" ] && [ "$(wc -c < "$out")" -ge 3000 ]; then
    echo "✅ $out ($(wc -c < "$out") 字节)"
  else
    echo "⚠ $bn 导出失败/过小 — 报告相应位置用占位符（[此处插入XX图]），不阻塞"
  fi
done
```
