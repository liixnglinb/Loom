...
echo ""
echo "=== TikZ diagrams ==="
# TikZ 图由 paper-figure-html 生成为 figures/tikz_diagrams.tex → 编译成 figures/tikz_diagrams.pdf
# （历史命名可能是 tikz_architecture_examples.tex，一并兼容）。
# TikZ 的 PDF 已经由 paper-figure-html 写进 latex_includes.tex，按 latex_includes.tex 嵌入即可。
...
- **TikZ diagrams must be embedded** into corresponding sections — every `tikz_*.pdf` referenced in `latex_includes.tex` must appear in some section (paper-figure-html already added include blocks for them)
