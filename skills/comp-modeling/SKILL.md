...
### Step 5.4: ⛔ 题意-模型对齐复核（建模完成后必做，防跑偏提前暴露）

**目的**：把"方向读歪"在建模步就地拦截，避免拖到逻辑复核重跑。逐条过一遍，不达标当场修模型再继续：

1. **子问题全覆盖**：赛题每个问题（问题一/二/三…）是否都有对应模型与结论输出？列出"题目问题 → 对应模型"映射清单，漏问立即补。
2. **目标对齐**：每个模型的优化目标/输出类型是否和题目要求一致（题目要"排序"就别给"分类"、要"预测"就别给"描述"）？
3. **约束对齐**：题设硬约束（容量/预算/时效/单位一致）是否全部进模型？与 `_utils/error_prevention.md` 该题型的"必须做/禁止做"逐条核对。
4. **假设可辩护**：每条假设都写出"为什么合理/忽略了什么"，不写套话。
5. **数据口径对齐**：模型输入与数据清单（data_clean.csv 字段）一一对应，无虚构变量。
6. **结果可达**：所选算法能算出该规模的结果（数据量/变量数/约束数），备选方案就绪。

复核结论写入 `MODELING_REPORT.md` 的「题意-模型对齐复核」小节（逐条 ✓/✗ + 修改记录），comp-code/review 步骤直接引用。
