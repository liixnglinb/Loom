...
def ensemble_prediction(predictions_dict, weights=None):
    ...
    Args:
        ...
        weights: dict 同名 keys, 若传入则按其归一化; 不传则退化为均匀权重,
                 实际使用时应由留出集误差倒数给出, 不能直接用均匀值凑数
    """
    if weights is None:
        # 占位退化: 均匀权重。正式使用时用留出验证误差倒数确定各子模型权重。
...
# ============================================================
# 6. SARIMA 季节ARIMA  ⭐ 国赛高频 (带周期/季节效应)
# ============================================================
def fit_sarima(y, order=(1, 1, 1), seasonal_order=(1, 1, 1, 12), forecast_steps=12):
    """
    拟合季节 ARIMA (SARIMA), 适用于带明显季节周期的时序(如以月为周期的销量/客流)。

    Args:
        y: ndarray (n,) 原始时序 (应含季节/周期成分)
        order: (p, d, q) 非季节部分 ARIMA 阶数
        seasonal_order: (P, D, Q, s) 季节部分阶数, s 为季节周期 (如 12 表示月度数据按年周期)
        forecast_steps: int 预测步数

    Returns:
        dict with keys: model, forecast, aic, bic
    """
    model = SARIMAX(
        y,
        order=order,
...
if __name__ == "__main__":
    ...
    # 6. SARIMA 季节预测 (y 已含 12 周期季节成分)
    sarima_result = fit_sarima(y[:48], order=(1, 1, 1),
                               seasonal_order=(1, 1, 1, 12), forecast_steps=12)
    print(f"SARIMA AIC: {sarima_result['aic']:.2f}, BIC: {sarima_result['bic']:.2f}")
    print(f"SARIMA 未来 12 期预测: {np.round(sarima_result['forecast'], 2)}")

    # SARIMA 预测 vs 真实
    fig_s, ax_s = plt.subplots(figsize=(10, 4))
    ax_s.plot(y, label="真实", marker='o')
    pred_steps = len(sarima_result["forecast"])
    ax_s.plot(np.arange(n, n + pred_steps), sarima_result["forecast"],
              label="SARIMA预测", marker='x', color='red')
    ax_s.axvline(n - 1, color='gray', ls='--', label="样本结束")
    ax_s.set_xlabel("时间"); ax_s.set_ylabel("y")
    ax_s.set_title("SARIMA 预测 vs 真实")
    ax_s.legend(); plt.tight_layout()
    plt.savefig("figures/prediction_sarima.png", dpi=300)
    print("已保存 figures/prediction_sarima.png")
