...
# ============================================================
# 7. 灰色关联分析 (Grey Relational Analysis, GRA)
# ============================================================
def grey_relational_analysis(X, reference=None, rho=0.5):
    """
    灰色关联分析: 通过比较序列与最优参考序列的几何形状相似度衡量关联程度。
    适于样本少、指标无量纲差异大、数据规律弱的小样本综合评价。

    Args:
        X: (m, n) 比较序列矩阵, m 个对象, n 个指标
        reference: (n,) 参考序列, 默认逐列取最大值 (正向理想取值)
        rho: 分辨系数, 通常取 0.5, 取值范围 (0, 1], 越小区分度越大
    Returns:
        dict with keys:
            relational_degree: (m,) 各对象关联度 (每指标关联系数的均值)
            correlation_matrix: (m, n) 各对象各指标关联系数
    """
    X = np.asarray(X, dtype=float)
    if X.ndim != 2 or X.shape[0] == 0:
        raise ValueError("X 必须是非空二维比较序列矩阵")
    m, n = X.shape
    if reference is None:
        reference = X.max(axis=0)   # 参考序列: 各指标最优取值
    reference = np.asarray(reference, dtype=float).ravel()
    if reference.shape[0] != n:
        raise ValueError("reference 长度必须等于指标数 n")

    # Step1 初值化: 每列除以该列第一个值, 消除量纲差异; 首值为 0 时用 1 兜底避免除零
    first_row = np.where(np.abs(X[0, :]) > 1e-12, X[0, :], 1.0)
    X0 = X / first_row
    X0_ref = reference / first_row

    # Step2 差序列与两级最小差 / 最大差
    delta = np.abs(X0 - X0_ref)
    min_delta = delta.min()
    max_delta = delta.max()

    # Step3 关联系数 (分辨系数 rho 控制分辨能力)
    correlation = (min_delta + rho * max_delta) / (delta + rho * max_delta)

    # Step4 关联度: 每个对象在所有指标上的关联系数取均值
    relational_degree = correlation.mean(axis=1)
    return {"relational_degree": relational_degree,
...
if __name__ == "__main__":
    ...
    # ---- 灰色关联分析 GRA 示例 ----
    # 比较序列: 5 个对象 × 5 个指标 (模拟, 实际从附件读入)
    plt.rcParams["font.sans-serif"] = ["SimHei", "Microsoft YaHei", "DejaVu Sans"]
    plt.rcParams["axes.unicode_minus"] = False
    X_gra = np.array([
        [0.90, 0.75, 0.62, 0.40, 0.58],
        [0.80, 0.70, 0.78, 0.55, 0.72],
        [0.60, 0.88, 0.90, 0.70, 0.65],
        [0.50, 0.60, 0.72, 0.85, 0.80],
        [0.70, 0.82, 0.66, 0.60, 0.68],
    ])
    ref_gra = X_gra.max(axis=0)   # 参考序列: 各指标最优取值
    gra = grey_relational_analysis(X_gra, reference=ref_gra, rho=0.5)
    gra_rank = np.argsort(-gra["relational_degree"])   # 降序
    print("\n灰色关联度:", np.round(gra["relational_degree"], 4))
    print("灰色关联排序 (对象):", gra_rank + 1)

    fig, ax = plt.subplots(figsize=(8, 4))
    ax.bar(range(1, X_gra.shape[0] + 1), gra["relational_degree"],
           color="steelblue")
    ax.set_xlabel("评价对象")
    ax.set_ylabel("灰色关联度")
    ax.set_title("灰色关联分析 -- 各对象关联度")
    ax.grid(axis="y")
    plt.tight_layout()
    plt.savefig("figures/evaluation_gra.png", dpi=300)
    print("已保存 figures/evaluation_gra.png")
