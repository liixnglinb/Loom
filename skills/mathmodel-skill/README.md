...
> A structured Agent workflow for CUMCM (全国大学生数学建模竞赛) — designed to keep a 72-hour modeling project coherent from the first decision to the final submission.
...
[![Competition](https://img.shields.io/badge/CUMCM-workflow-f97316)](./competitions/)
...
| 竞赛要求逐年变化，维护成本不断增加           | 规则集中在 `competitions/mathmodel/`，主流程不含竞赛特化逻辑       |
...
本 skill 专注于 CUMCM 国赛，规则差异集中在 `competitions/<comp>/`、题型与阶段权重、LaTeX 模板、提交与披露规则几处，保持单一来源，避免多竞赛定义漂移。
...
CUMCM 分位描述的是公开样本中的观察位置，不是官方评分线，也不能用于推导获奖概率，不会生成缺乏数据支持的“经验分位”。
...
CUMCM 会根据是否使用 AI 生成相应的支撑材料 PDF 或正文声明，避免截止前再依赖记忆补写。
...
| -------------- | ------------------------------------------- | ------------------------------------------------------------ | ----------------------------------------------------------- |
| **CUMCM 国赛** | 中文；XeLaTeX / 原创 `ctexart` 电子论文模板 | 收集 91 份公开论文源样本，其中 59 份成功提取文本并进入统计；57 项维护者反模式检查 | 观察分位不是官方门槛，规则以当届通知为准                    |
...
| 工具                         | 用途                                                  | 典型调用                                                     |
| ---------------------------- | ----------------------------------------------------- | ------------------------------------------------------------ |
| `scripts/doctor.py`          | 检查 skill 结构、竞赛包、环境与工作区                 | `python <skill>/scripts/doctor.py --competition mathmodel`   |
...
| `scripts/render_paper.py`    | 将标准 Markdown 工作区装配为国赛的 TeX/PDF        | `python <skill>/scripts/render_paper.py --competition mathmodel --workspace paper_workspace` |
| `scripts/render_ai_usage.py` | 根据台账生成 CUMCM AI 使用披露材料                | `python <skill>/scripts/render_ai_usage.py ...` |
...
- 国赛生成的 section 会自动接入 `main.tex`。缺失章节、空章节、未知 marker 或重复 marker 都会明确失败，正式编译不会静默降级。
...
- 新增 AI 使用台账与披露生成器：CUMCM 根据“已使用 / 明确未使用”生成支撑材料 PDF 或正文声明。
- 修复分类交叉验证中的标准化泄漏、优化示例中的不可行贪心基线、熵权法常数列、GM(1,1) 极限和 MAPE 零值边界。
- 将 CUMCM 样本口径校准为“91 份来源、59 份可提取文本”；Stage 9 动态加载对应反模式清单，不再在共享状态中写死数量。
...
python scripts/doctor.py --competition mathmodel --skip-tools
```
...
仓库原创代码、文档以及国赛装配模板采用 [MIT License](./LICENSE)。运行时依赖和外部资料链接仍遵循各自的许可条款，详细边界见 [`THIRD_PARTY_NOTICES.md`](./THIRD_PARTY_NOTICES.md)。
