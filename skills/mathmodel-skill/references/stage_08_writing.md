...
`01_abstract.md` contains abstract/summary content without a top-level heading because the template supplies its wrapper. Files `02`–`10` each own one clear top-level Markdown heading; the template intentionally does not print duplicate body headings.

Write the body first, then references and appendices, and write the abstract/summary last. Every number in the abstract must point to a result already present in the body. The abstract follows the team skeleton: one background paragraph (3–4 lines), one paragraph per subproblem starting with 针对问题X (6–8 lines each, method then results, include validation when available), and a final keywords line; the total must just fill the first A4 page.
...
- English abbreviations give their full forms at first occurrence, then stay abbreviated;
- inline citations appear only in the problem analysis and at first method introduction, never in the abstract, results, or conclusions;
- no extra spaces follow Chinese punctuation;
- single figures are about 70% of the text width and are scaled without distortion;
- section numbering follows 一、→ 1.1 → bullet; level-1 titles are centered with fonts larger than the body, level-2 titles are left-aligned, and 1.2 问题提出 opens with an intro paragraph followed by one bullet per complete subproblem;
- the paper uses a split-level structure: each subproblem gets its own level-1 chapter (六、问题一模型建立及求解, 七、问题二模型建立及求解, …), each in three parts X.1 模型建立 / X.2 求解算法 (with a flowchart) / X.3 求解结果与分析; data problems add a level-1 五、数据预处理 chapter before the modeling chapters;
- figures are routed by type per `competitions/<comp>/plotting_routing.md`: data/statistical charts use modelviz-skill, flow/gantt/architecture/mindmap charts use multi-chart-draw-skills; figures are optional and added only when they make the point clearer, never to inflate count.
- long tables that span pages (notably the 符号说明 table) follow the three-line pagination rule: every page keeps the top rule, header mid rule, and bottom rule; every non-final page ends with a bottom rule plus small right-aligned "续下页" text. Use the four-part `longtable` template in `templates/shared/notation_table.md` §跨页规范 (see anti-pattern I12).
- every page's left and right edges align to the text block: tables use `p{…\textwidth}` columns summing to `\textwidth` with `\tabcolsep=0`; long inline formulas at sentence end get `\allowbreak` at relational operators; the template enables `\emergencystretch`; appendix code lines are manually wrapped to the line width (see anti-pattern I13).
...
The renderer assembles CUMCM directly. A generated PDF with missing section inputs is a failure even if LaTeX exits successfully.
