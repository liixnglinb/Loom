...
> CUMCM 国赛通用 5 维 rubric (共享 stage 0-7 框架, stage 8/9 由 `competitions/<comp>/rubric_overlay.json` 特化)。L1 Critic 直接 JSON 化使用。
...
| 层级 | 来源 | 加载 |
|------|------|------|
| 通用基础 | 本文件 stage 0-9 表格 | CUMCM 共享 |
...
```json
{
  ...
  "verdict": "block | pass_early | pass | pass_with_review | refine | refine_partial"
}
```
