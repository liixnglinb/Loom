...
Competition: {competition} (mathmodel)
...
def verdict(scores, issues, weights=None):
    """
    weights: 题型 dim 权重表 (e.g. config/dim_weights.json[mathmodel][A_optimization]["3"]).
...
`decision_log.task_type` 由 stage 1 选题后填入 (e.g. `A_optimization` / `C_data`)。`score_artifact.py` 加载 `config/dim_weights.json[competition][task_type]` 拿到 stage→dim→weight 表, 应用到 verdict 计算。
...
**示例** (mathmodel/C_data, stage 6):
> 本题为 mathmodel/C_data, 重点考察: 验证设计与数据风险匹配 (×1.4), 输出完备性 (×1.2). 其他维度按默认 1.0 评估。旧维度键可保留，但不把多变量参数数量当作质量代理。
...
elif raw_min >= 9 and weighted_mean >= 9:
                      verdict = "pass_early"       # 全 Qi pass 且达标即早退, 对应 score_artifact.compute_stage5_verdict
