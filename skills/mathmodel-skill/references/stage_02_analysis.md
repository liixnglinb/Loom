...
论文归位说明："问题重述"只保留"1.1 问题背景"与"1.2 问题提出"两个小节；"问题分析"采用逐问式（2.1 问题一的分析 ~ 2.4 问题四的分析，每问逻辑：题意拆解 → 难点剖析 → 模型选型与适配 → 求解思路简述，纯文字不写公式），本步的"数据流与依赖关系"与 Step 4 的"统一口径/数据 schema"内容归入问题分析；数据题还需把"数据概览/缺失与异常处理/特征工程/描述性统计与可视化"组织为独立一级章"五、数据预处理"。
