...
- 论文结构约定：每个 Qi 在论文中对应**独立一级章**（如"六、问题一模型建立及求解"），内部按 X.1 模型建立（决策变量/目标/约束/模型汇总）、X.2 求解算法（选型理由+步骤+流程图）、X.3 求解结果与分析（数据表+可视化+解读）三段式组织
...
入口：python Q1_solve.py；依赖：numpy/pandas/cvxpy
"""
import numpy as np
import pandas as pd
import cvxpy as cp
...
print("Q1 求解完成, 结果已保存至 results/Q1_x.npy")
```
...
5. 触发 L2: 跨阶段回检 stage 2 (子问题分解是否成立) + stage 3 (模型选择前提是否被结果推翻) + stage 4 (符号一致性) + **review_qis 列表 (若 verdict=pass_with_review)**
