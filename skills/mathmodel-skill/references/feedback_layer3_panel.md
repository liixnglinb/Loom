...
> **id 来源唯一**：panelist 的唯一标识（`id`）与权重一律取自 `competitions/<competition>/rubric_overlay.json["panel_personas"]`（权威主数据），不要用本文档的语义章节名（如 Mathematical rigor）或 persistence 示例里的键名当 persona id。CUMCM 当前主数据的 `id` 均为 `math_rigor / model_innovation / code_correctness / writing_clarity / judge_30s`；聚合权重查找以 overlay 的 `id` + `weight` 为准。下列 5 节是这些 persona 的职责说明，标题仅作可读标签。
...
### CUMCM AI-compliance reviewer

Add this as an independent reviewer in Stage 9 for CUMCM (alongside the standard five). It walks the full 26-item checklist in `competitions/mathmodel/ai_compliance_checklist.md` — 2026 AI new-rules, AI-hallucination/data truthfulness, modeling logic/problem fit, solving/result plausibility, official writing rules, and layout/page count — and marks each `合格 / 存在问题 / 待核实` with a location and fix. It returns the same shared JSON schema (scores on five dimensions + issues + verdict), and any fatal item (AI-led core modeling, fabricated data/literature, or an AI-usage declaration contradiction) forces `block`, overriding panel aggregation.
