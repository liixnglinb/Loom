...
在国赛题号体系内 (mathmodel A-E), 选出**最契合团队优势 + 时间预算 + 数据可获取性**的一题，并让选择、否决与后续变更都有据可查。
...
# 加载后得到国赛的题号清单 (mathmodel A-E) 与每题的 task_type_key
```
...
|---|---|---|---|
| mathmodel | A 优化, B 评价, C 数据, D 工业, E 创新 | 3-5 | 中文论文 |
...
**同步写 root 字段**:
- `decision_log.task_type` ← `topic_specs.json[selected].task_type_key` (e.g. `A_optimization` for mathmodel-A)
