...
1. **竞赛** — 固定为 `mathmodel 国赛`
2. **题号** — 依国赛题号体系动态生成选项 (mathmodel A-E / `未公布`)
...
**自动推断** (加载 `competitions/mathmodel/README.md` 与 `topic_specs.json`):
- 时长预算 (mathmodel 72h)
- 写作语言 (中文)
- LaTeX 编译器 (xelatex)
...
python <skill>/scripts/doctor.py --competition mathmodel --workspace .
...
# LaTeX 必备
xelatex --version          # CUMCM ctexart 模板使用 xelatex
...
|---|---|---|---|
| mathmodel | `<skill>/templates/latex/mathmodel/main.tex` | xelatex | 91 份来源记录 / 59 份可提取样本观察 |
