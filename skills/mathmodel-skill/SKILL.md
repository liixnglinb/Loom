...
description: CUMCM 国赛(全国大学生数学建模竞赛)的端到端协作工作流。Use when a user explicitly works on the CUMCM contest or asks to run/review a CUMCM modeling-competition paper from problem selection through modeling, solving, robustness, writing, compliance, and final submission review. Provides 10 stages, persistent decision state, competition-specific rules/templates, deterministic scoring helpers, numbered decisions, and Codex/Claude Code handoff. Do not trigger for generic model selection, ordinary data analysis, or non-competition paper review.
---

# mathmodel-skill — 国赛(CUMCM)数学建模工作流 (v6.1)

10 阶段把 72 小时的竞赛协作变成可恢复、可检查的流程。用户回答关键问题，agent 维护状态与脚本。每阶段产出经过 rubric 自评、定向精修与跨阶段一致性回检；Stage 8–9 先遵守当届官方规则，再做多视角终审。CUMCM 包含 91 份来源文档，其中 59 份进入文本统计，经验统计为观察值而非分数线。

**v6.1 更新**: 加入竞赛规则基线与 AI 使用披露链路；统一使用 marker 模板并对提交元数据 fail closed；修复状态路径错位、评分 verdict 持久化、题型权重合并和 YAML frontmatter 等问题；新增 preflight doctor 与自动化验证。仅支持数学建模(国赛 mathmodel)竞赛。
...
| **竞赛特化** | `competitions/<comp>/...` 按 decision_log.competition dispatch | `competitions/mathmodel/winning_patterns.md` |
| **LaTeX 模板** | `templates/latex/<comp>/main.tex` | `templates/latex/mathmodel/main.tex` |
...
约定: `<skill>/` = skill 安装目录, `<cwd>/` = 用户 cwd, `<comp>/` = 当前竞赛 (mathmodel)。
...
|---|---|---|---|---|---|
| mathmodel | 72h | 中文 | xelatex / 原创 ctexart | CUMCM 2026 | 91 来源文档 / 59 可提取样本 |

当前仅支持国赛 mathmodel；经验数据为观察分位，不得解释为官方阈值或获奖预测。
...
| 1 | 选题 (多题对比 → 1) | `stage_01_problem_selection.md` | 2-4h | L1 | 题号体系 (A-E) + task_type 写入 |
...
| 6 | 全局灵敏度 / 稳健性 | `stage_06_robustness.md` | 2-3h | L1 + L2 | 数学参数 |
...
- **stage 8 经验锚点**: `competitions/<comp>/empirical.json` 只作评分前参考；CUMCM 为 59 份可提取样本的观察分位，不得推断为数值门槛
...
**Stage 9 的 `block` / `refine` 特判**：Stage 9 的合规 block（含 26 项 AI 自查清单致命违规）不是"直接暂停交付给用户"，而是触发"不合规 → 回退重改 → 重审"闭环（见 `references/stage_09_review.md` §6b）：按问题归属退回 Stage 3/5/8 或渲染器修复，改完重进 Stage 9 只重跑受影响的检查，直到 `submission_ready=true` 且无致命违规。此闭环只能在 Stage 9 内执行，不改变其它阶段的 `block`=暂停语义。
...
- "切到 mathmodel" / "重做 stage N" → 改 decision_log 或跳转
...
- `competitions/mathmodel/`: 91 份来源文档，59 份成功文本提取并进入观察分位；现有提取有局限，不能解释为官方阈值或获奖预测
- 通用模型清单 `references/model_catalog.md` 跨竞赛复用

当前 `scripts/ingest_papers.py` 是维护期归档工具，不能直接重建 `empirical.json`。新增语料前先补来源 provenance、提取 QA 与分组样本量。
