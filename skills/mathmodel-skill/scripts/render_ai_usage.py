...
Outputs are placed where the paper renderer expects them:
  * mathmodel with AI use: ``support_materials/AI工具使用详情.{md,pdf}``
  * mathmodel without AI use: ``paper_workspace/AI工具未使用声明.md``

ReportLab is imported only for the MATHMODEL PDF.  Use ``--markdown-only`` to
inspect MATHMODEL Markdown without creating the PDF.
"""
...
MATHMODEL_MARKDOWN_FILENAME = "AI工具使用详情.md"
MATHMODEL_PDF_FILENAME = "AI工具使用详情.pdf"
MATHMODEL_NO_USE_FILENAME = "AI工具未使用声明.md"

REPORTLAB_ERROR = (
    "生成 MATHMODEL《AI工具使用详情.pdf》需要 ReportLab。"
...
def resolve_decision_log_path(cli_arg: Optional[str] = None) -> Path:
    """Resolve CLI > MATHMODEL_STATE_DIR > MATHMODEL_STATE_DIR > cwd/state."""
    if cli_arg:
        return Path(cli_arg)
...
def resolve_competition(cli_arg: Optional[str], decision_log: Dict[str, Any]) -> str:
    ...
    if competition != "mathmodel":
        raise LedgerValidationError(
            "AI 使用报告目前仅支持 competition='mathmodel'（CUMCM 国赛）；"
...
def render_mathmodel_markdown(
    entries: Sequence[AIUsageEntry], decision_log: Dict[str, Any]
) -> str:
    if not entries:
        raise LedgerValidationError(
            "MATHMODEL 空台账应生成参考文献后的未使用声明，而不是 AI 工具使用详情"
        )
    lines = ["# 人工智能工具使用详情", ""]
...
def render_mathmodel_no_use_statement() -> str:
...
def render_mathmodel_pdf(
    ...
    if not entries:
        raise LedgerValidationError(
            "MATHMODEL 未使用 AI 时只需在参考文献后声明，不应生成使用详情 PDF"
        )
    rl = _load_reportlab()
    colors = rl["colors"]
    A4 = rl["A4"]
...
def render_reports(
    ...
    *,
    ...
    if not entries:
        for stale_path in (
            support_dir / MATHMODEL_MARKDOWN_FILENAME,
            support_dir / MATHMODEL_PDF_FILENAME,
        ):
            stale_path.unlink(missing_ok=True)
        paper_workspace.mkdir(parents=True, exist_ok=True)
        statement_path = paper_workspace / MATHMODEL_NO_USE_FILENAME
        statement_path.write_text(render_mathmodel_no_use_statement(), encoding="utf-8")
        return [statement_path]

    support_dir.mkdir(parents=True, exist_ok=True)
    (paper_workspace / MATHMODEL_NO_USE_FILENAME).unlink(missing_ok=True)
    md_path = support_dir / MATHMODEL_MARKDOWN_FILENAME
    md_path.write_text(
        render_mathmodel_markdown(entries, decision_log), encoding="utf-8"
    )
    outputs = [md_path]
    if not markdown_only:
        outputs.append(
            render_mathmodel_pdf(
                entries, decision_log, support_dir / MATHMODEL_PDF_FILENAME
            )
        )
    else:
        (support_dir / MATHMODEL_PDF_FILENAME).unlink(missing_ok=True)
    return outputs
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--markdown-only",
        action="store_true",
        help="MATHMODEL 仅生成 Markdown，不生成 PDF",
    )
    return parser
