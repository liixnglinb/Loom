...
# MATHMODEL-exclusive files (2026 AI compliance checklist applies only to mathmodel).
MATHMODEL_EXTRA_FILES = ("ai_compliance_checklist.md",)
COMPETITION_FILES = (
    "README.md",
    "winning_patterns.md",
    "phrase_bank.md",
...
RENDER_ENGINES = {"mathmodel": "xelatex"}
REQUIRED_TEX_FILES = {
    "mathmodel": ("ctexart.cls",),
...
EXPECTED_RENDER_MARKERS = {
    "mathmodel": CORE_SECTION_MARKERS | {"mathmodel_no_ai_statement"},
...
def run_checks(
    ...
    required_paths = (
        ...
        "templates/latex/mathmodel/main.tex",
    )
    ...
    if competition == "mathmodel":
        missing_comp += [name for name in MATHMODEL_EXTRA_FILES if not (comp_dir / name).is_file()]
    checks.append(_check(
        "competition-pack",
        not missing_comp,
        f"{competition}: {len(COMPETITION_FILES) + (len(MATHMODEL_EXTRA_FILES) if competition == 'mathmodel' else 0)} required files"
...
def main() -> int:
    ...
    parser.add_argument("--competition", choices=COMPETITIONS, default="mathmodel")
