"""
score_artifact.py — L1 Critic 输出后的本地处理脚本 (v3.1 国赛版)
...
路径协议:
- decision_log: 默认 <cwd>/state/decision_log.json, 可用 MATHMODEL_STATE_DIR (兼容老 MATHMODEL_STATE_DIR) 或 --decision-log 覆盖
- competition: 默认从 decision_log.competition 读, 缺失则 mathmodel; 可用 --competition 或 MATHMODEL_COMPETITION env 覆盖
...
用法:
    ...
    python scripts/score_artifact.py --competition mathmodel --task-type A_optimization --stage 3 --critique c.json
    # stage 5 per-Qi 评分:
...
COMPETITIONS = {"mathmodel"}

# Baseline DIM_WHITELIST (mathmodel 基线; 可通过 rubric_overlay.json 的 dim_whitelist 覆盖)
DIM_WHITELIST = {
...
def resolve_decision_log_path(cli_arg: str = None) -> Path:
    """路径解析协议: CLI > MATHMODEL_STATE_DIR > MATHMODEL_STATE_DIR (兼容) > <cwd>/state/decision_log.json"""
    if cli_arg:
        return Path(cli_arg)
    env_dir = os.environ.get("MATHMODEL_STATE_DIR") or os.environ.get("MATHMODEL_STATE_DIR")
    if env_dir:
...
def validate_critique(critique: dict, stage_id: int, competition: str = "mathmodel",
