"""Build descriptive statistics from a local directory of MATHMODEL PDFs.
...
automatically update ``competitions/mathmodel/empirical.json``.

Dependencies:
    python -m pip install -r scripts/requirements-maintenance.txt

Example:
    python scripts/ingest_papers.py --papers-dir /path/to/mathmodel-papers \\
...
def render_markdown(overall: dict, by_year: dict, by_letter: dict, n_total: int) -> str:
    lines = []
    lines.append("# MATHMODEL 可提取样本观察分布 (empirical_distribution)")
    lines.append("")
    lines.append(f"> 基于输入目录中 {n_total} 份可提取文本的 MATHMODEL 论文样本自动生成。")
    ...
    lines.append("- 更新 `competitions/mathmodel/empirical.json` 前，必须人工复核样本构成、异常值和当年规则。")
    lines.append("")
    return "\n".join(lines)
