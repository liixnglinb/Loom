...
- `paper-figure-html` step appends architecture/flow/pipeline figure blocks (fig_roadmap.pdf, fig_flow_*.pdf, tikz_*.pdf, etc.)
...
**⛔ Every figure block from `latex_includes.tex` MUST appear in some section file** — both data figures (Results section) and HTML 引擎 architecture diagrams (Methods/Overview sections). Missing any figure = audit fail.
