...
# 赛项标识：从 CLAUDE.md「赛项」行读取（Additional Parameters 已被运行时净化，不可再依赖）；缺省回退国赛
COMPETITION=$(grep -m1 '赛项' CLAUDE.md 2>/dev/null | sed -E 's/.*赛项[：:][[:space:]]*//; s/[[:space:]（(].*//' | tr -d '\r')
COMPETITION="${COMPETITION:-国赛}"



if [ "$COMPETITION" = "stats" ] || echo "$COMPETITION" | grep -qi "统计建模\|stats"; then

    echo "Using stats template"

    cp "$TMPL_BASE/stats/"* paper/ 2>/dev/null

elif echo "$COMPETITION" | grep -qi "apmcm_zh\|亚太.*中文\|亚太赛中文" || grep -qi "apmcm_zh\|亚太.*中文\|亚太赛中文" CLAUDE.md 2>/dev/null; then

    echo "Using APMCM (Chinese) template (MathorCupmodeling.cls)"

    cp "$TMPL_BASE/apmcm_zh/"* paper/ 2>/dev/null

elif echo "$COMPETITION" | grep -qi "mathorcup\|MathorCup\|mathor" || grep -qi "mathorcup" CLAUDE.md 2>/dev/null; then

    echo "Using MathorCup template"

    cp "$TMPL_BASE/mathorcup/"* paper/ 2>/dev/null

elif echo "$COMPETITION" | grep -qi "huazhong\|华中杯" || grep -qi "huazhong\|华中杯" CLAUDE.md 2>/dev/null; then

    echo "Using huazhong template"

    cp "$TMPL_BASE/huazhong/"* paper/ 2>/dev/null

elif echo "$COMPETITION" | grep -qi "huawei\|华为杯" || \
...
elif echo "$COMPETITION" | grep -qi "wuyi\|五一杯" || grep -qi "wuyi\|五一杯" CLAUDE.md 2>/dev/null; then

    echo "Using wuyi template"

    cp "$TMPL_BASE/wuyi/"* paper/ 2>/dev/null

elif echo "$COMPETITION" | grep -qi "mcm\|icm\|美赛" || grep -qi "mcm\|icm\|美赛" CLAUDE.md 2>/dev/null; then

    echo "Using MCM template"

    cp "$TMPL_BASE/mcm/"* paper/ 2>/dev/null

elif echo "$COMPETITION" | grep -qi "cumcm\|国赛" || grep -qi "cumcm\|国赛" CLAUDE.md 2>/dev/null; then

    echo "Using cumcm template"

    cp "$TMPL_BASE/cumcm/"* paper/ 2>/dev/null

elif echo "$COMPETITION" | grep -qi "changsanjiao\|长三角" || grep -qi "changsanjiao\|长三角" CLAUDE.md 2>/dev/null; then

    echo "Using changsanjiao template"

    cp "$TMPL_BASE/changsanjiao/"* paper/ 2>/dev/null

elif echo "$COMPETITION" | grep -qi "huashu\|华数杯" || grep -qi "huashu\|华数杯" CLAUDE.md 2>/dev/null; then

    echo "Using huashubei template"

    cp "$TMPL_BASE/huashubei/"* paper/ 2>/dev/null

elif echo "$COMPETITION" | grep -qi "diangong\|电工杯" || grep -qi "diangong\|电工杯" CLAUDE.md 2>/dev/null; then

    echo "Using diangongbei template"

    cp "$TMPL_BASE/diangongbei/"* paper/ 2>/dev/null

elif echo "$COMPETITION" | grep -qi "dongsansheng\|东三省\|辽宁" || grep -qi "dongsansheng\|东三省\|辽宁" CLAUDE.md 2>/dev/null; then

    echo "Using dongsansheng template"

    cp "$TMPL_BASE/dongsansheng/"* paper/ 2>/dev/null

elif echo "$COMPETITION" | grep -qi "shuwei\|数维杯" || grep -qi "shuwei\|数维杯" CLAUDE.md 2>/dev/null; then
    ...
    cp "$TMPL_BASE/cumcm/"* paper/ 2>/dev/null    # 兜底：确保 paper/main.tex 必然存在
...
# TikZ 由 paper-figure-html 生成为 figures/tikz_diagrams.tex → 编译成 figures/tikz_diagrams.pdf（多图则 tikz_*.pdf / tikz_diagrams_N.pdf）
...
**⛔⛔⛔ HTML 引擎 图嵌入规则（最容易被遗漏，必须逐条检查）：**



HTML 引擎 图（技术路线图、求解流程图、Pipeline 图等）的 `\begin{figure}...\end{figure}` 代码块在 `latex_includes.tex` 的**末尾部分**（由 paper-figure-html 步骤追加）。你必须把它们嵌入到正确的章节：



| HTML 引擎 图类型 | 嵌入位置 | 章节文件 |
...
**写完所有章节后，必须运行以下检查确认 HTML 引擎/TikZ 图全部嵌入：**

```bash

echo "=== HTML 引擎/TikZ 嵌入检查 ==="
...
- **⛔ 禁止使用 LaTeX `subfigure`/`subcaption` 并排「多张独立 PDF」。** 每张图必须独占一个 `\begin{figure}[H]...\end{figure}` 环境，宽度 ≥ `0.85\textwidth`。不要用 `\begin{subfigure}{0.48\textwidth}` 把两张独立 PDF 缩成半页宽——这会导致图片太小看不清。如果确实需要对比两张图，分成两个独立的 figure 环境，中间加 2-3 句过渡分析文字。
  ...
  **⛔ 但逻辑框图例外中的例外——流程图/架构图/技术路线图/TikZ 几何·算法·架构示意图（html 或 tikz 生成的逻辑框图）绝对单列独占，不论主题多相关都不并排**：它们靠节点文字和连线表意，缩成半栏字就糊了，并排没有信息收益。上面的✅ matplotlib 合成单 PDF 只针对**数据结果图**（柱状/折线/热力/混淆矩阵等）的并排对比，逻辑框图（含 tikz_*.pdf）不走这条、永远一张一行。
