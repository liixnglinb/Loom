...
Generate figures and tables from data: **Data source: 当前工作区 RESULTS.md / 数据文件**
...
- **FORMAT = `pdf` 主引用 + `svg` 主存档**（二者均矢量、放大不糊，论文里每一张图都是 SVG 级矢量）：
- 出图用 `save_fig(fig, 'figures/fig_xxx.pdf')` —— plot_utils 自动**并列写一份同名 `.svg` 主存档**；
- `\includegraphics` 引用 `.pdf`（矢量）；**禁止 `.png/.jpg/.jpeg` 进论文**（compile_check 会硬拦）。
...
⛔ **Step 0 · 图表选型（写脚本前强制执行）**：动手写 `gen_fig_*.py` 之前，
先读 `_utils/chart_select_guide.md`（三阶段选型，已随工作区注入）+ `_utils/chart_library.md`（52 类图目录）：
...
⛔ **数据图命名规范**：本步骤画的是**数据图**（柱状/折线/热力/散点等），命名 `fig_<语义>`，但**避开架构/流程图专用前缀**：`fig_arch` / `fig_flow` / `fig_roadmap` / `fig_pipeline` / `fig_framework` / `fig_network` / `fig_state` / `fig_decision` / `fig_overview` 等（这些归 paper-figure-html）。例如流速图用 `fig_velocity` 而非 `fig_flow_rate`、状态分布用 `fig_status_dist` 而非 `fig_state_traj`。否则本步骤的产出对账会把它当架构图跳过，导致漏画不报错。
...
if [ -n "$PLAN_FILE" ]; then
  ...
  # ⛔ 只对「数据图」硬对账。流程/架构/路线图和 tikz_ 由 paper-figure-html 子流程产出，
  ...
  for name in $EXPECTED_FIGS; do
    ...
    ls figures/${name}.png figures/${name}.pdf figures/${name}.html 2>/dev/null | head -1 | grep -q . || MISSING_FIGS="$MISSING_FIGS $name"
  ...
  if [ "$MISSING_COUNT" -gt 0 ]; then
    ...
    echo "✅ FIGURE_MANIFEST 数据图全部产出: $TOTAL_EXPECTED 张(流程/架构图归 paper-figure-html)"
...
HAS_DRAWIO=$(ls figures/fig_*.html 2>/dev/null | wc -l)
...
echo "  fig_*.png: $HAS_PNG, fig_*.pdf: $HAS_PDF, tikz_*.pdf: $HAS_TIKZ, fig_*.html: $HAS_DRAWIO"
...
if [ -z "$PLAN_FILE" ]; then
  ...
  # ⛔ 拆成「数据图」与「流程/架构图(html/tikz)」两类。本步骤只对数据图硬对账，

  # html/tikz 类由 paper-figure-html / TikZ 子流程负责，缺它们不算本步骤失败(否则空转)。
  ...
  [ -n "$(echo "$DRAWIO_FIGS" | grep .)" ] && { echo "  流程/架构图(由 paper-figure-html 负责, 本步骤不检查)："; echo "$DRAWIO_FIGS" | grep . | sed 's/^/    /'; }



  # 逐条检查数据图产物是否存在 (任意一种格式都算: .png / .pdf / .html)
  ...
  if [ "$MISSING_COUNT" -gt 0 ]; then
    ...
    echo "  ✅ 数据图全部产出(流程/架构图归 paper-figure-html)"
...
| FIGURE_MANIFEST 对账显示 **MISSING_COUNT > 0** | ⛔⛔⛔ **逐张补齐**：每个缺失的 `fig_xxx` 必须按其在规划文档里的描述去生成。`fig_flow_q*` / `fig_roadmap` / `fig_pipeline` 等 html 类的 → 调 paper-figure-html；`tikz_*` → 调 TikZ 子流程；其余数据图 → Step 3 写 `gen_fig_xxx.py` 执行画图 |
...
| `TOTAL_FIG < PLAN_FIG_COUNT` 且 `gen_fig_*.py == 0` | **图全是 html 流程图，缺核心数据图**。检查 `figures/*.json` 数据，写 gen_fig 脚本补齐 |
...
3. `figures/*_results.json` ≥ 1 个 但 `figures/fig_*.png/pdf` 全是 html 流程图 → 「核心数据图没生成」
...
- **html 流程图不能替代数据结果图**：竞赛论文要求技术路线图（html）+ 子问题求解流程图（html）+ **数据结果图（matplotlib gen_fig）**，三类都要有
...
1. Read the full style guide (color schemes + figure selection decision table + anti-patterns + HTML 引擎/TikZ color schemes — all in one file):
...
[ ] 6. roadmap — Technical roadmap (HTML 引擎)

Total planned: 6 figures + 2 tables + 1 HTML 引擎
...
5. **Always generate** HTML 引擎 architecture diagrams
...
GPT Image 2 can generate high-quality scene diagrams, technical roadmaps, flowcharts, and architecture diagrams — far better than HTML 引擎.
...
**直接调用即可。成功就用，失败 3 次后 HTML 引擎 兜底。不需要检测 Python 或检查环境变量。**
...
3. 如果 3 次重试全部失败 → 记录到 `_gptimg_failed.txt` → 由 paper-figure-html 步骤自行选择最合适的替代方案（HTML 引擎 或 TikZ，根据图的内容自主判断）
...
if [ "$GPTIMG_PLAN_COUNT" -gt 0 ]; then
    ...
    echo "   失败 3 次后才允许降级（HTML 引擎 或 TikZ，自行判断哪个更合适）"
...
**GPT Image 只用于场景示意图（物理/工程类赛题的问题背景图）。技术路线图、求解流程图、模型架构图使用 HTML 引擎。**
...
if [ -f figures/fig_roadmap.pdf ]; then
    ...
    echo "❌ fig_roadmap FAILED after 3 retries — will use HTML 引擎 fallback"
...
**7. Record failures for HTML 引擎 fallback (persist to file for paper-figure-html step).**
...
if [ "$GPT_IMG" -eq 0 ]; then
    ...
    echo "⚠ 所有 GPT Image 图都失败了，可能是 API Key 未配置或网络问题，HTML 引擎 将生成所有替代图"
...
- `ALL_SUCCESS` → all GPT Image figures generated, HTML 引擎 only generates figures NOT in the GPT Image plan

- `SOME_FAILED` → HTML 引擎 generates replacements ONLY for the failed figures

- `ALL_FAILED` → all attempts failed (API Key missing / network error), HTML 引擎 generates ALL non-data figures

- `GPT_IMG_DISABLED` → Python not found, HTML 引擎 generates ALL non-data figures
...
- 失败的图记录到 GPTIMG_FAILED，HTML 引擎 子阶段会自动兜底
...
# ⛔ 门 2：快速模式让位。省额度优先，即使开了数据图 vision 也跳过
  ...
  #    数据图 = matplotlib gen_fig 脚本产的；流程/架构图有同名 .html（归 paper-figure-html 的
  ...
  #      负向兜底：无 .html、无 tikz .tex、非 GPT 前缀 → 可能是「一脚本产多图」的数据图，也检
  ...
  for pdf in figures/fig_*.pdf; do
    ...
    if [ -f "figures/gen_${bn}.py" ]; then
      ...
      # 兜底：排除 html 流程图 / TikZ / GPT Image 插画，其余当数据图（覆盖一脚本多图）

      _skip=0

      [ -f "figures/${bn}.html" ] && _skip=1
...
Save to `figures/latex_includes.tex`. **Figures引用只写 `.pdf`（矢量，源为同存 `.svg` 主存档）；论文内不允许出现 `.png/.jpg` 引用**——
compile_check.sh 的「图必须矢量」闸会拒绝任何位图入论文。Figures use `[H]` float specifier (pinned in place to prevent multi-figure stacking); tables use `[H]` (requires `\usepackage{float}`).
...
for plan in PROBLEM_ANALYSIS.md TOPIC_PLAN.md PAPER_PLAN.md MODELING_REPORT.md; do
    ...
    grep -E 'fig_|TABLE_|HTML 引擎|TikZ|GPTIMG|数据图|图表' "$plan" | head -30
...
echo "HTML 引擎 diagrams:"

ls -1 figures/*.html 2>/dev/null && echo "YES" || echo "NO"
...
for plan in PAPER_PLAN.md PROBLEM_ANALYSIS.md TOPIC_PLAN.md MODELING_REPORT.md; do
    ...
    [ -f "$plan" ] && echo "--- $plan ---" && grep -i 'fig\|图\|table\|表\|chart\|plot\|heatmap\|radar\|HTML 引擎\|html\|TikZ\|tikz' "$plan" | head -30
...
[✅] 6. roadmap — 技术路线图 → figures/fig_roadmap.html + figures/fig_roadmap.pdf (exists)
...
5. **⛔ 如果某张图反复失败（同一工具 3 轮都不行），启用跨工具兜底：**
   ...
   - HTML 引擎 失败 → 降级到 TikZ（简化版）

   - TikZ 失败 → 降级到 HTML 引擎（去掉公式，用文字代替）

   - GPT Image 失败 → 降级到 HTML 引擎（已有机制）
...
# 3. HTML 引擎 diagrams (if planned)

if grep -qi 'html\|HTML 引擎\|架构图\|技术路线\|roadmap\|framework\|流程图' PAPER_PLAN.md TOPIC_PLAN.md PROBLEM_ANALYSIS.md 2>/dev/null; then

    # HTML 引擎/TikZ 检查已移至 paper-figure-html 步骤，此处跳过

    DRAWIO_COUNT=$(ls figures/*.html 2>/dev/null | wc -l)

    [ "$DRAWIO_COUNT" -gt 0 ] && echo "  (HTML 引擎: $DRAWIO_COUNT files — will be validated by paper-figure-html step)" || echo "  (no HTML 引擎 yet — will be generated by paper-figure-html step)"
...
# 4.5 TikZ/HTML 引擎 — handled by paper-figure-html step, skip here

echo "  (TikZ/HTML 引擎 diagrams will be generated and validated by the next step: paper-figure-html)"
...
if [ "$GPTIMG_PLANNED" -gt 0 ]; then
    ...
    if [ "$GPTIMG_PDF" -gt 0 ]; then
        ...
        # Check if HTML 引擎 fallback was used

        echo "  GPT Image: no PDFs (may have used HTML 引擎 fallback — check GPTIMG_FAILED)"
...
# Check for oversized PDFs (HTML 引擎/TikZ/GPT Image figures that might be too tall)
...
- HTML 引擎 .html files export to PDF via `screenshot_capture.py --file … --out figures/xxx.pdf --format pdf`（同 paper-figure-html）
