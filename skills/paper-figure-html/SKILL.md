...
用 HTML+CSS 生成论文非数据类示意图：**主题以上一步建模/分析产物及当前论文上下文为准**

这是从 paper-figure 拆出的**轻量子步骤**，是 `paper-figure-html` 的 **HTML 平替**（用户可二选一，HTML 为默认）。只处理架构/流程/路线类示意图，数据图（matplotlib/seaborn）已在前一步 paper-figure 生成。

**HTML 相对 HTML 引擎 的核心优势**：用 flex/grid 自动布局，不写绝对坐标 → 天然免疫节点重叠/坐标错位/连线穿越。因此本 skill **不需要** html 的坐标结构自检（html_pdf_check.py），改用 HTML/PDF 专属质检。
...
# 视觉自检脚本（复用 tikz_vision_check.py，与画图引擎无关；不存在则跳过视觉自检）
VISION=""
...
# tikz_vision_check.py 视觉自检（与 tikz_vision_check 同源，接受 PNG）
# ⛔ 三处查找（对齐 html 侧 Step 7.5）：_utils/（后端正常复制的位置）→ $MH_TOOLS_DIR
...
**必须产出至少 1 张 `figures/fig_*.pdf`，并更新 `figures/latex_includes.tex`**。产物契约与 paper-figure-html **完全一致**（后端按同一口径对账，两个 skill 可互换）：
...
if [ -n "$PLAN_FILE" ]; then
    ...
    # ⛔ 按 manifest「HTML/HTML 引擎 章节」标题抓该章节下的全部图名（权威），不靠文件名前缀白名单
    EXPECTED=$(printf '%s\n' "$MANI" | awk '
        /^[[:space:]]*\*\*/ { cap = (tolower($0) ~ /html|html|tikz/) ? 1 : 0; next }
...
echo "=== 规划中的架构/流程图清单 ==="
grep -A 60 -iE 'HTML|HTML 引擎|架构图|流程图|技术路线' "$PLAN_DOC" 2>/dev/null | grep -E '^\- \[ \]? *fig_(arch|flow|roadmap|pipeline|framework)' || echo "（未找到显式清单，按工作流类型决定）"
...
- `--format pdf`（或 out 以 .pdf 结尾）→ 量内容真实像素、页面设成刚好等于内容 → **单页、无白边、真矢量**（文字可选可搜、无限放大不糊），等效 html `--crop`。
...
### Step 5: 视觉自检（vision LLM，复用 tikz_vision_check，⛔ 不阻塞）

html_pdf_check 只看 PDF 结构，看不出渲染后的视觉效果（文字挤、配色刺眼等）。这一步用 vision LLM 真正"看图"。**复用** `tikz_vision_check.py`（它接受 PDF/PNG，与画图引擎无关）。**FAST_MODE=1 时跳过本步。**
...
if [ "$FAST_MODE" = "1" ]; then
  ...
  echo "🟥 vision 工具(tikz_vision_check.py)不可用，HTML 流程/架构图未做视觉审查（环境限制，不硬拦）"
...
2. 按 vision 反馈改（HTML 是相对布局，改法比 html 简单）：
...
NEED_TIKZ=0
# ⛔ 判据①：manifest/规划里出现 tikz_ 图名 = 硬需求（与 Step 6 对账、html 侧同口径）
...
⛔ **失败兜底**：HTML 引擎**没有 html 可退**。若某公式图 3 轮编不出，**大幅精简**（去掉次要标注、拆成两张更简单的图、公式改行内文字描述）再试；仍不行则**保留其余已成功产物**，在 latex_includes.tex 该图位置写一行 `% TODO: tikz_xxx 编译失败，需人工补` 注释，**不阻塞整步结束**。
...
#    收集到才检、收集为空才真跳过（对齐 html 侧 Step 7.5 的稳健口径）。NEED_TIKZ 仅
...
if [ "$FAST_MODE" = "1" ]; then
  ...
  for pdf in figures/*.pdf; do
    ...
    #    tikz_diagrams_N.pdf 带 tikz_ 前缀，已被上一行前缀判断覆盖，无需回退（对齐 html 侧 7.5）。
...
" 2>/dev/null && [ -f "_tmp/${bn}_v.png" ] && PNG_OK=1
      ...
      # 第三级兜底 pdf2image（对齐 html 侧 Step 7.5，多一条转换路径）
...
#   - unresolved：审了、3 轮没修好 → 计入 GATE_FAIL，硬拦（与 html 引擎对称，默认引擎不再静默放行）
...
规划步骤（paper-plan 等）在规划文档里维护 `<!-- BEGIN FIGURE_MANIFEST -->` 区块，本 skill 据此对账。章节标题格式与 html 版保持一致，只把 "HTML 引擎" 字样改成 "HTML"。后端按**粗体章节标题里的关键词**归类（`html` 或 `html` 都归到本 skill/-html 这一类，两者互换），不看文件名前缀，所以 `fig_data_pipeline` 这类"关键词在中间"的名字也不会漏。

⛔ **几何图（TikZ）的 manifest 归属**：HTML 引擎下 TikZ 也由**本 skill** 产出，所以 TikZ 图名要放进**含 "HTML" 字样的章节**（或单独写一个标题里带 "HTML/TikZ" 的章节），这样后端才把它归到本 skill 的对账通道（后端第一优先匹配标题里的 `html`/`html` 关键词）。⛔ **不要**沿用 html 版把 TikZ 单列成 `**TikZ 图（paper-figure 产出）：**`——那个标题会被后端归到 `paper-figure`（数据图）通道，导致本 skill 产出的 tikz 图对不上账。
...
- HTML 用 flex/grid 自动布局，**不写绝对坐标** → 免疫重叠/错位/连线穿越（相对 html 的核心优势）。
...
- ⛔ TikZ 图的 manifest 章节标题要带 "HTML" 字样（归本 skill 对账通道），别用 html 版的独立 "TikZ 图" 标题。
