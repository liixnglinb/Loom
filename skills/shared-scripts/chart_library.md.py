...
> 确定 1 个最终图型后，再按「画法出处」打开对应 recipe（或 plot_utils 函数）抄代码、填真实数据。
...
> ⛔ 抄 recipe 代码时：`figsize` 不要照抄原文占位值，按 `_utils/figure_check.sh` / `fig_include_size.py` 的「长宽比档位」换算（单 panel 横图约 6.0in、2×2 近方图约 5.0in，详见 figure_recipes_competition.md 开头）。

## 使用方式（强制流程）

1. **Step 1 · 数据判定**：根据手上数据的形态 + 论文要回答的问题，先定「类别」（比较/趋势/分布/相关/组成/专业/示意），见 chart_select_guide.md。
...
| 图名 | 适配判定 | 画法出处 |
|---|---|---|
| 分组柱状图 | 2-5 组方法 × 多指标，要精确数值+误差棒 | recipe basic#1 / plot_utils.bar_compare |
| 堆叠柱状图 | 构成占比随时间/条件堆叠，总量+分项 | recipe basic#2 |
| 发散条形图 | 双向差异（正负两侧对称展示，如盈亏/偏差） | recipe advanced#20 |
| 水平条形图 | 类别名长/类目多（≥6 项）排序展示 | plot_utils（barh）或 basic 风格 |
...
| 直方图 | 单变量分布，观察偏态/峰态 | plot_utils.distribution_plot |
| 密度曲线 | 连续分布对比，平滑形态 | plot_utils（kdeplot） |
...
| 图名 | 适配判定 | 画法出处 |
|---|---|---|
| 散点图·回归 | 两连续变量关系+拟合线+r² | recipe basic#4 / plot_utils.scatter_plot |
...
| 组合子图 | 多 panel 统一叙事（2×2 等） | recipe basic#12 / plot_utils.subplot_grid |
...
- plot_utils 现成函数优先：`bar_compare / heatmap / forest_plot / trend_plot / scatter_plot / distribution_plot / multi_line_plot / box_plot / radar_plot / subplot_grid`。
