# -*- coding: utf-8 -*-
"""ModelFlow 智模流水线 · 桌面应用启动入口（纯桌面形态）。
...
def _alert(msg: str):
    ...
    try:
        import ctypes
        ctypes.windll.user32.MessageBoxW(None, msg, "ModelFlow", 0x10)
    except Exception:
        pass
...
def main():
    ...
    print("ModelFlow 智模流水线 启动: http://127.0.0.1:" + str(port))
    try:
    ...
    def _toggle_autostart():
        ...
        cur = None
        try:
            cur, _ = winreg.QueryValueEx(key, "ModelFlow")
        ...
        if cur:
            winreg.DeleteValue(key, "ModelFlow")
        else:
            winreg.SetValueEx(key, "ModelFlow", 0, winreg.REG_SZ, cmd)
        winreg.CloseKey(key)
