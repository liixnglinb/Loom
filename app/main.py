# -*- coding: utf-8 -*-
"""ModelFlow 智模流水线 主入口：竞赛工作流全自动流水线。"""
from pathlib import Path
from fastapi import FastAPI, UploadFile, File
from fastapi.staticfiles import StaticFiles
from fastapi.responses import FileResponse, JSONResponse
from pydantic import BaseModel
from . import db, jobs, llm, paths
from .ws import router as ws_router
import time, threading, json

app = FastAPI(title="ModelFlow 智模流水线")
db.init_db()
...
def create_wf(w: WorkflowIn):
    wid = db.create_workflow(w.title, w.template, w.step, w.config)
    return {"id": wid}
...
def rerun_step(wid: int, q: RerunStepIn):
    ...
    tpl = w.get("template") or "competition"
    step_list = jobs.PIPELINES.get(tpl, jobs.COMPETITION_STEPS)
...
def list_pipelines():
    """返回全部流水线的元数据 + 步骤列表（skill/label/out/checkpoint/role）。"""
    out = []
    for tpl, meta in PIPELINE_CATALOG.items():
        steps = jobs.PIPELINES.get(tpl, [])
        out.append({"template": tpl, **meta,
                    "steps": [{"key": s["key"], "label": s["label"], "skill": s["skill"],
                               "out": s["out"], "checkpoint": s.get("checkpoint", False)}
                              for s in steps]})
    comp = [{"key": s["key"], "label": s["label"], "skill": s["skill"], "out": s["out"],
             "checkpoint": s.get("checkpoint", False)} for s in jobs.COMPETITION_STEPS]
    return {"pipelines": out, "competition": {"template": "competition", "name": "竞赛工作流", "steps": comp}}
