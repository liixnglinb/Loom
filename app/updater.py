...
两种运行形态（2026-08-28 起支持装机版）：
  ...
  装机版：Inno Setup 装到 %LOCALAPPDATA%\Programs\ModelFlow，Uninstall.exe 同目录。
...
def new_exe_path() -> Path:
    return update_dir() / "ModelFlow-new.exe"
...
def _write_update_bat(old_pid: int) -> Path:
    ...
    exe = Path(sys.executable).resolve() if paths.FROZEN else \
        (paths.BASE / "dist" / "ModelFlow.exe").resolve()
...
:waitloop
tasklist /FI "PID eq %OLD_PID%" 2>nul | find /I "ModelFlow" >nul
if not errorlevel 1 (
...
def apply() -> dict:
    ...
    装机版（paths.INSTALLED）：下载物重命名为 ModelFlow-{ver}-setup.exe 后
    静默升级安装（Inno 覆盖程序文件、保留 data/ 与快捷方式），
    /restart 跳过运行中检测；本进程 1.2s 后退出为文件覆盖让路，
    Inno [Run] 段的 {app}\\ModelFlow.exe --relaunch 会自动把新版拉起来。
    便携版：沿用二段式 bat 原地替换。
    """
    if paths.FROZEN:
        new = new_exe_path()
        ...
        if paths.INSTALLED:
            ...
            setup = update_dir() / f"ModelFlow-{ver}-setup.exe"
            new.replace(setup)
