...
# 步骤编排：配置驱动 —— 不再有硬编码的 COMPETITION_STEPS / PIPELINES 常量。
# 每个工作流实例创建时从所选模板快照其「有序步骤清单」存进 workflows.steps_snapshot，
# 执行引擎只读实例自己的快照，因此「步骤数/步骤名/每步 skill/模型」完全由配置决定，
# 编辑模板不影响任何已创建的实例。

def resolve_steps(w):
    """从工作流实例读取步骤清单（快照优先，兼容无快照的旧数据回退到内置 competition 模板）。

    返回有序步骤 dict 列表。每步: key,label,skill,out,checkpoint,role,check[,model]。
    """
    snap = (w or {}).get("steps_snapshot")
    if isinstance(snap, list) and snap:
        return [dict(s) for s in snap]
    # 兼容：没有快照时，按 template 名从数据库模板取（极少发生，退化路径）
    tpl = (w or {}).get("template") or "competition"
    p = db.get_pipeline(tpl)
    if p and p.get("steps"):
        return [dict(s) for s in p["steps"]]
    # 最坏回退：内置极速流
    return [dict(s) for s in db.get_pipeline("competition")["steps"]] if db.get_pipeline("competition") else []


def is_competition_template(template):
    """判断是否为竞赛类工作流（决定 agent 措辞 / 模板注入 / 默认条款）。

    配置驱动下改为读模板分组：模板在 pipeline_definitions 里带 g 分组（comp=竞赛）。
    返回 True 表示按竞赛语义处理。模板不存在时按名字宽松回退（兼容旧名）。
    """
    t = (template or "").strip()
    if not t:
        return False
    if t in ("competition", "competition_bzd", "competition_mathmodel") or t.startswith("comp_"):
        return True
    p = db.get_pipeline(t)
    if p:
        return (p.get("g") or "").strip() in ("comp", "competition")
    return False
...
# ⚠️ 已收敛为配置驱动：本表定义迁至 app/pipelines.py builtin_pipelines() 作为 seed 入库，
#    用户可在软件里把它「另存为副本」并编辑步骤。此处不再硬编码。
# 步骤编排完全配置驱动：PIPELINES / COMPETITION_STEPS_BZD / COMPETITION_STEPS_MMS /
# STEP_INDEX / is_competition_template 等硬编码常量全部移除。
# 三套内置模板（competition / competition_bzd / competition_mathmodel）的定义
# 已收敛到 app/pipelines.py 的 builtin_pipelines() 作为 seed 数据入库，
# 执行引擎统一走上面 resolve_steps() 从实例快照读步骤，不再区分模板类型。
...
def prepare_cli_workspace(ws, config, file_index, step_list=None, current=None, template="competition"):
    ...
    # 2) CLAUDE.md：流水线概览 + 规则 + 上传文件清单 + 当前步骤的 SKILL 全文
        _fallback_steps = (db.get_pipeline("competition") or {}).get("steps") or []
        labels = " > ".join(s["label"] for s in (step_list or _fallback_steps))
        cur_block = ""
    if current:
...
def execute_job(wid):
    """后台运行：按工作流实例自己的步骤快照执行全部步骤（第 4/5 步并行），更新数据库进度。"""
    ...
    # 配置驱动：从实例快照读步骤清单（不再是硬编码 PIPELINES）
    step_list = resolve_steps(w)
    # 开关门控（兼容现有 UI 表单）：logic_review 未开启 → 跳过「逻辑对抗复核」步骤（review）
